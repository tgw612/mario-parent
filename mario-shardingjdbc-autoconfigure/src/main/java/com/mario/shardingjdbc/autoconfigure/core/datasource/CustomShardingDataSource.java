...
public class CustomShardingDataSource extends AbstractDataSourceAdapter implements AutoCloseable {
  ...
  public CustomShardingDataSource(Map<String, DataSource> dataSourceMap, ShardingRule shardingRule)
      throws SQLException {
  ...
  public CustomShardingDataSource(Map<String, DataSource> dataSourceMap, ShardingRule shardingRule,
    ...
    this.shardingProperties = new CustomShardingProperties(
  ...
  private void checkDataSourceType(Map<String, DataSource> dataSourceMap) {
    ...
    while (var2.hasNext()) {
      ...
      Preconditions.checkArgument(!(each instanceof MasterSlaveDataSource),
  ...
  private ShardingContext getShardingContext(ShardingRule shardingRule) throws SQLException {
    int executorSize = (Integer) this.shardingProperties
        .getValue(CustomShardingPropertiesConstant.EXECUTOR_SIZE);
    int maxConnectionsSizePerQuery = (Integer) this.shardingProperties
        .getValue(CustomShardingPropertiesConstant.MAX_CONNECTIONS_SIZE_PER_QUERY);
    boolean showSQL = (Boolean) this.shardingProperties
        .getValue(CustomShardingPropertiesConstant.SQL_SHOW);
    ShardingExecuteEngine executeEngine = new ShardingExecuteEngine(executorSize);
    if (executorSize > 0) {
      int queueSize = (Integer) this.shardingProperties
          .getValue(CustomShardingPropertiesConstant.EXECUTOR_QUEUE_SIZE);
      ListeningExecutorService newExecuteEngine = this
          .getNewListeningExecutorService(executorSize, queueSize);
      this.resetListeningExecutorService(executeEngine, newExecuteEngine);
    }

    return new ShardingContext(this.dataSourceMap, shardingRule, this.getDatabaseType(),
        executeEngine, maxConnectionsSizePerQuery, showSQL);
  }
  ...
  private void resetListeningExecutorService(ShardingExecuteEngine executeEngine,
      ListeningExecutorService newExecutorService) {
    ...
    if (listeningExecutorServiceField == null) {
      ...
      try {
        ...
        throw new RuntimeException("Can not set field[executorService] in ShardingExecuteEngine.",
            var6);
      }
    }
  }
  ...
  private ListeningExecutorService getNewListeningExecutorService(int executorSize, int queueSize) {
    ...
    ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(executorSize, executorSize, 0L,
        TimeUnit.MILLISECONDS, new LinkedBlockingQueue(queueSize),
        ShardingThreadFactoryBuilder.build());
