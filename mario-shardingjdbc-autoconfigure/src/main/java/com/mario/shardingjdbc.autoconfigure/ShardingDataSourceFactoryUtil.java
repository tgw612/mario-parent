...
public class ShardingDataSourceFactoryUtil {
  ...
  public static DataSource createShardingDataSource(
      ...
      List<MasterSlaveRuleConfiguration> masterSlaveRuleConfigurationList,
      Map<String, DataSource> dataSourceMap) throws SQLException {
    //数据分片 加 读写分离
    //分配规则配置
    ShardingRuleConfiguration shardingRuleConfig = shardingProperties
        .getShardingRuleConfiguration();
    //主从规则配置
    shardingRuleConfig.setMasterSlaveRuleConfigs(masterSlaveRuleConfigurationList);

    CustomShardingProperties customShardingProperties = new CustomShardingProperties(
        shardingProperties.getProps());

    int executorSize = customShardingProperties
    ...
    if (executorSize > 0) {//如果大于0
      ...
      return CustomShardingDataSourceFactory
          .createDataSource(dataSourceMap, shardingRuleConfig, shardingProperties.getConfigMap(),
              shardingProperties.getProps());
    } else {
      //原逻辑
