...
public class ShardingLazyDataSource extends AbstractUnsupportedOperationDataSource implements
    AutoCloseable {
  ...
  ShardingLazyDataSource(Map<String, DataSource> dataSourceMap,
    ...
    this.threadPoolExecutor = new ThreadPoolExecutor(threadsSize, threadsSize, 0L,
        TimeUnit.MILLISECONDS, new LinkedBlockingQueue(), DouboThreadFactory
    ...
    this.boss = new Thread(() -> {
      try {
        ...
        try {
          this.targetDataSource = ShardingDataSourceFactoryUtil
    ...
    while (var5.hasNext()) {
      ...
      this.threadPoolExecutor
          .execute(new ShardingLazyDataSource.TaskThread((DruidDataSource) entry.getValue()));
    }
