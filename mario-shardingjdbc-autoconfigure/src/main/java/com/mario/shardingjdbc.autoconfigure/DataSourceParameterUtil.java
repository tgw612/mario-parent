...
public final class DataSourceParameterUtil {
  ...
  public static DataSourceParameter getDataSourceParameter(String dataSourceClassName,
      Map<String, Object> dataSourceProperties) throws ReflectiveOperationException {
    DataSourceParameter dataSourceParameter = (DataSourceParameter) Class
        .forName(dataSourceClassName).newInstance();
    return getDataSourceParameter(dataSourceParameter, dataSourceProperties);
  }

  public static DataSourceParameter getDataSourceParameter(DataSourceParameter dataSourceParameter,
      Map<String, Object> dataSourceProperties) throws ReflectiveOperationException {
    Iterator var2 = dataSourceProperties.entrySet().iterator();

    while (var2.hasNext()) {
      Entry<String, Object> entry = (Entry) var2.next();
      callSetterMethod(dataSourceParameter, getSetterMethodName((String) entry.getKey()),
  ...
  private static String getSetterMethodName(String propertyName) {
    return propertyName.contains("-") ? CaseFormat.LOWER_HYPHEN
        .to(CaseFormat.LOWER_CAMEL, "set-" + propertyName)
        : "set" + String.valueOf(propertyName.charAt(0)).toUpperCase() + propertyName
            .substring(1, propertyName.length());
  }

  private static void callSetterMethod(DataSourceParameter dataSource, String methodName,
      String setterValue) {
  ...
  static {
    generalClassType = Sets.newHashSet(
        new Class[]{Boolean.TYPE, Boolean.class, Integer.TYPE, Integer.class, Long.TYPE, Long.class,
            String.class});
  }
}
